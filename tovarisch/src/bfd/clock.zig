// clock.zig — Testable monotonic clock abstraction
//
// Provides a Clock trait and real implementation for BFD timers.
// The abstraction allows unit tests to control time progression.
...
/// This is appropriate for BFD timers since monotonic time is not affected
...
fn currentMonotonicMillis() MonoTime {
    // On Linux, use CLOCK_MONOTONIC for BFD timers.
    // On non-Linux (macOS, etc.), return 0 and let tests use MockClock.
    // Note: tovarisch targets Linux-only for now.
...
/// Real monotonic clock for BFD timers.
/// Uses CLOCK_MONOTONIC so NTP adjustments don't affect BFD detection timers.
