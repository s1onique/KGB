#!/usr/bin/env python3
"""Self-tests for CI baseline validation. Core logic in verify_memory_budgets_ci.py."""
...
def run_ci_baseline_self_tests() -> bool:
    ...
    valid_artifact = {
        ...
        "environment": {"arch": "linux/amd64", "environment_label": "github_hosted_ubuntu",
            "_github_workflow_run": 28013478727, "_github_artifact_id": 7815736588,
            "_github_artifact_name": "tovarisch-memory-lab"},
        "workload": {"type": "idle-warmup", "operations": 0, "errors": 0, "duration_ms": 60000},
        "memory": {"first": {"rss_kib": 8000}, "max": {"rss_kib": 8200},
            "last": {"rss_kib": 8100}, "growth": {"rss_kib": 100}},
        "decision": {"pass": True, "reason": "test"}
    }
    
    valid_budget = {
        "version": "1.0", "service": "tovarisch",
        "ci_idle_baselines": {
            "github_hosted_ubuntu": {
                "idle": {"rss_kib": 8200, "pss_kib": 6200},
                "evidence_sources": [{"workflow_run": 28013478727, "artifact_id": 7815736588,
                    "artifact_name": "tovarisch-memory-lab", "service_version": "0.1.0",
                    "service_commit": "abc123", "workload": "tovarisch-idle-warmup",
                    "duration_seconds": 60, "environment_label": "github_hosted_ubuntu"}]
            }
        }, "arch_budgets": {},
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Test 1: Valid CI baseline passes
        ...
        # Test 3: Schema fixture fails
        with open(artifact_path, "w") as f:
            json.dump(dict(valid_artifact, evidence_kind="schema_fixture"), f)
        errors = check_ci_baseline_evidence_exists(valid_budget, "test.yaml", tmpdir)
        ...
        env = dict(valid_artifact["environment"], environment_label="router_armv7")
        with open(artifact_path, "w") as f:
            json.dump(dict(valid_artifact, environment=env), f)
        errors = check_ci_baseline_evidence_exists(valid_budget, "test.yaml", tmpdir)
        ...
        with open(artifact_path, "w") as f:
            json.dump(dict(valid_artifact, service={"name": "uvb76", "version": "1.0.0", "commit": "xyz789"}), f)
        errors = check_ci_baseline_evidence_exists(valid_budget, "test.yaml", tmpdir)
        ...
        # Test 7: Mismatched artifact_id fails
        env = dict(valid_artifact["environment"], _github_artifact_id=9999999999)
        with open(artifact_path, "w") as f:
            json.dump(dict(valid_artifact, environment=env), f)
        errors = check_ci_baseline_evidence_exists(valid_budget, "test.yaml", tmpdir)
...
if __name__ == "__main__":
    # Import and run long-window tests too
    from verify_memory_budgets_leak_slope_long_window_tests import run_long_window_self_tests
    success = run_ci_baseline_self_tests() and run_long_window_self_tests()
