...
Examples:
  ...
  python3 tovarisch_status_rss_canary.py \\
    ...
    if args.warmup_requests < 0:
        print("ERROR: warmup-requests must be non-negative", file=sys.stderr)
        sys.exit(3)
    if args.sample_requests < 0:
        print("ERROR: sample-requests must be non-negative", file=sys.stderr)
        sys.exit(3)
    if args.interval_seconds < 0:
        print("ERROR: interval-seconds must be non-negative", file=sys.stderr)
        sys.exit(3)
    if args.timeout_seconds <= 0:
        print("ERROR: timeout-seconds must be positive", file=sys.stderr)
        sys.exit(3)
    if args.max_rss_kib_growth < 0:
        print("ERROR: max-rss-kib-growth must be non-negative", file=sys.stderr)
        sys.exit(3)
    if args.max_private_kib_growth < 0:
        print("ERROR: max-private-kib-growth must be non-negative", file=sys.stderr)
        sys.exit(3)
