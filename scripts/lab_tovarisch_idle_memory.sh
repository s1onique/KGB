...
Subsystem toggles (SYNTHETIC EVENT EMISSION ONLY - do NOT disable actual tovarisch paths):
...
main() {
    ...
    echo "Subsystem toggles (SYNTHETIC only - no actual runtime effect):"
...
EOF
    ...
    if [[ -n "${TOVARISCH_WG_COMMAND_PATH:-}" ]]; then
        TOVARISCH_WG_COMMAND_PATH="${TOVARISCH_WG_COMMAND_PATH}" "${TOVARISCH_BINARY}" serve --bind "${LAB_BIND}:${LAB_PORT}" &
    else
    ...
    verdict=$(python3 "${ANALYZER_SCRIPT}" "${artifact_path}/memory_samples.tsv" "${artifact_path}" "${artifact_path}/event_timeline.tsv" \
        --duration "${DURATION}" --heartbeat-enabled "${HEARTBEAT_ENABLED}" --wg-enabled "${WG_CHECK_ENABLED}" --bgp-bfd-enabled "${BGP_BFD_ENABLED}" 2>&1)
